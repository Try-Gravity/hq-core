...
- **New core script `core/scripts/hq-services.sh`** (`search|browse|info`) and skill `/hq-services`: HQ's read-only client for [Gravity Index](https://index.trygravity.ai). When a task needs a developer service the project does not have (database, auth, hosting, payments, email, monitoring), sessions search the Index for a recommendation with integration steps and a tracked setup link instead of picking a vendor from memory. Calls go to hq-pro `/services/*` with the caller's HQ Cognito token (`identity-resolve.sh`); hq-pro forwards to the Index with HQ's publisher key, so no Gravity credential is ever configured on a user's machine. Account provisioning is not included; the user follows the setup link and stores keys with `/hq-secrets`. Server contract for hq-pro: `core/knowledge/public/hq-core/hq-services-proxy-spec.md`. Regression: `core/scripts/tests/hq-services.test.sh`. Charter, quick reference, and user guide point at the new path.
